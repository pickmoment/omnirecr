...
export const ScriptLibrary: React.FC<ScriptLibraryProps> = ({
  ...
  const flash = (msg: string) => {
    setFeedback(msg);
    setErrorMsg(null);
    window.setTimeout(() => setFeedback((cur) => (cur === msg ? null : cur)), 2600);
  };
  ...
  const handleDelete = async (item: ScriptItem) => {
    if (!window.confirm(`"${item.title}" 대본을 삭제할까요? 되돌릴 수 없습니다.`)) return;
    try {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 h-full min-h-0">
      ...
      <div className="flex-1 min-w-0 min-h-0 flex flex-col gap-3">
        ...
        {selected?.last_recorded_path && (
          <div className="bg-indigo-950/30 border border-indigo-800/50 rounded-2xl p-3 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[11px] font-bold text-indigo-300">최근 녹음 결과</p>
              <p className="text-[11px] text-slate-400 font-mono truncate">
                {selected.last_recorded_path}
              </p>
              {selected.last_recorded_at && (
                <p className="text-[10px] text-slate-500 mt-0.5">{selected.last_recorded_at}</p>
              )}
            </div>
            <button
              onClick={() => onOpenExplorer(selected.last_recorded_path!)}
