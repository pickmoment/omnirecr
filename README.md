...
- **🤖 선택한 대본 자동 일괄 녹음 (Batch Automation)**:
  ...
  - **재생 클릭 재시도**: 한 번 눌러서는 Typecast 가 반응하지 않아 소리가 안 나는 경우가 있어 기본 3회를 누르고, 재생이 확인되면 남은 클릭은 생략합니다 (횟수 조절 가능)
  - **소리 기반 시작/종료 판정**: 시스템 오디오 레벨로 낭독 시작을 감지하고, 설정한 시간만큼 무음이 이어지면 자동 종료 (Typecast가 어떤 방식으로 재생하든 동작)
  - **시작 전 사전 점검**: Typecast 로그인 여부 확인, 제목이 같아 서로 덮어쓰는 대본 검출, 이미 있는 결과 파일의 덮어쓰기를 한 번에 확인 (실행 도중에는 창을 가리는 확인 창이 뜨지 않습니다)
  - **녹음 중단 감지**: 오디오 캡처가 끊기면 무음으로 오해하지 않고 즉시 실패로 표시 — 잘린 파일이 대본에 연결되지 않습니다
  - 대본별 실시간 진행 상태(대기 · 입력 중 · 녹음 중 · 완료 · 실패 · 건너뜀)와 전체 진행률 표시, 언제든 중단 가능
